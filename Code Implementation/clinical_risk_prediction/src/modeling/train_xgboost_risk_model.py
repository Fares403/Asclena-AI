...
The model learns binary risk_target, but the system output is risk_score:
...
def create_predictions_table(conn: "Connection", sql_path: Path) -> None:
...
def save_predictions(
    ...
    with raw_conn.cursor() as cursor:
        cursor.execute(
            """
            DELETE FROM asclena.risk_predictions
            WHERE model_name = %s
              AND model_version = %s
            """,
        ...
        execute_values(
            cursor,
            """
            INSERT INTO asclena.risk_predictions (
              stay_id,
              subject_id,
              model_name,
              model_version,
...
def train_and_evaluate(args: argparse.Namespace) -> None:
    ...
    test_risk_scores = model.predict_proba(X_test_imputed)[:, 1]
    ...
    all_risk_scores = model.predict_proba(X_all_imputed)[:, 1]
    ...
    if args.save_predictions:
        ...
        with engine.connect().execution_options(isolation_level="AUTOCOMMIT") as conn:
            create_predictions_table(conn, sql_path)
            save_predictions(
                conn,
                args.schema,
                prediction_df,
